...
class HealthCheck {
	...
	*/
	function add_wp_tests( $tests ) {
		...
		foreach ( $this->requirements as $key => $required_tests ) {
			...
			if ( ! in_array( $key, array( 'versions', 'functions', 'constants', 'callbacks') ) ) continue;
	...
	*/
	function site_health_check_functions(){
		...
		$result = array(
	        'label'       => __( 'Required functions or modules tested.', 'trustedlogin-vendor' ),
	        'status'      => 'good',
	        'badge'       => array(
	            'label' => __( 'TrustedLogin', 'trustedlogin-vendor' ),
	            'color' => 'green',
	        ...
	        'description' => sprintf(
	            '<p>%s</p>',
	            __( 'TrustedLogin requires a few PHP and WordPress modules to work securely. These were all found and tested.', 'trustedlogin-vendor' )
	        ),
	        'actions'     => '',
