...
trait Debug_Logging {
	 ...
	 */
	static $DIRECTORY_PATH = 'trustedlogin-logs/';

	static $logging_threshold = 'debug';
	 ...
	 * @return false|Logger
	 */
	private function setup_klogger() {
		...
		try {
			...
			$default_options = array(
				...
				'filename'       => sprintf( 'trustedlogin-vendor-debug-%s-%s', wp_date( 'Y-m-d' ), wp_hash( $filename_hash_data ) ),
	...
	public function is_enabled() {
		...
		if ( property_exists( $this, 'settings' ) ) {
			$is_enabled = (bool) $this->settings->debug_mode_enabled();
		} elseif ( property_exists( $this, 'debug_mode' ) ) {
			$is_enabled = (bool) $this->debug_mode;
		}
		 ...
		 */
		$is_enabled = apply_filters( 'trustedlogin/vendor/logging/enabled', $is_enabled );
	...
	public function log( $message = '', $method = '', $level = 'debug', $data = array() ) {
		...
		if ( null === $this->klogger ) {
			$this->klogger = $this->setup_klogger();
		}
